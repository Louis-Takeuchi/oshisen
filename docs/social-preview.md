...
- 候補者は同じカード・配色で扱い、賛否を善悪の色に置き換えない。政策回答は一問ずつ示し、総合一致率・近さの順位・人物の点数は表示しない。通常の候補者一覧・比較は五十音順を基本にし、保存は支持表明として扱わない。
...
以下は現在のロゴ・共有カードを制作した時点の記録です。研究対応の新仕様は [research-design.md](research-design.md) を優先します。既存共有画像内の「政策で出会い、人柄で興味を深める。」は制作当時のコピーで、政策の総合点や人格の採点を行うという意味ではありません。
